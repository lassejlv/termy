...
| C, C++, Swift, Objective-C | `termy_core::ffi` | Opaque handles and a C-compatible ABI |
...
cd termy
cargo build --release -p termy_core
```

The build produces `libtermy_core.dylib` on macOS,
`libtermy_core.so` on Linux, or `termy_core.dll` on Windows under
`target/release`. The public header is
[`crates/core/include/termy.h`](https://github.com/lassejlv/termy/blob/main/crates/core/include/termy.h).

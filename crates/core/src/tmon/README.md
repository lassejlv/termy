...
Tmon is the terminal engine module at `crates/core/src/tmon/`. Its implementation
uses the standard library and direct platform APIs, with no GPUI dependency.
The enclosing core package owns its host integrations and Cargo dependencies.
...
The cross-engine benchmark lives in `crates/cli/examples/engine_compare.rs`.
Run it with `just benchmark-tmon`.
...
`target/benchmarks/tmon-alacritty-benchmark.txt`:
...
`main` that change its declared Cargo, core, CLI, or
...
set. Both `just benchmark-tmon` and CI append an allocation report to the timed comparison. Manual inputs are bounded so an oversized request

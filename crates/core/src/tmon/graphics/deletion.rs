...
impl GraphicsState {
    pub(super) fn delete(&mut self, command: &GraphicsCommand, grid: &Grid) -> ApplyResult {
        let (cursor_col, cursor_row) = grid.cursor_position();
        let alternate = grid.alternate_screen_mode();
        let history_size = if alternate { 0 } else { grid.history_size() };
        ...
        if !matches!(
            ...
            return failure(command, "EINVAL:unsupported delete selector");
        }
        ...
        let removed: HashSet<_> = self
            ...
            .iter()
            .filter(|placement| {
                if placement.alternate != alternate && !matches!(selector, 'i' | 'n' | 'r') {
                    return false;
                }
        ...
        self.placements
            .retain(|placement| !removed.contains(&placement.serial));
        ...
        success_with_resolved_image(command, self.placements.len() != before, id)
    }
}
